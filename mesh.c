...
for (i=0; i<95; i++){
    ...
    for (i=0; i<21; i++){
     ...
     /*First side*/
            ...
            flag=0;
            if (i==0){
                    nodes_up[j][0]=(nodes[res1-1][0] + nodes[res2-1][0])/2;
                    nodes_up[j][1]=(nodes[res1-1][1] + nodes[res2-1][1])/2;
                    j++;
                ...
                for (k=0; k<i; k++){
                    if ( (side1[k][0]==res2 && side1[k][1]==res1)  ){
                        flag=1;
                    }
                    else if( (side2[k][0]==res2 && side2[k][1]==res1)  ){  
                        printf("found on side2 line 62\n");
                    }
                    else if( (side3[k][0]==res2 && side3[k][1]==res1)  ){  
                ...
                if (flag==1){
                    ...
                    side4[47-i][0]=nodes_ptr[res2-1][res1-1];
                    side4[47-i][1]=res2;
                }
                else{
                    nodes_up[j][0]=(nodes[res1-1][0] + nodes[res2-1][0])/2;
         ...
         /*Side two */
            ...
            if (i==0){
                ...
                for (k=0; k<i; k++){
                    if ( (side2[k][0]==res2 && side2[k][1]==res1)  ){
                        flag=1;
                    }
                    else if( (side1[k][0]==res2 && side1[k][1]==res1)  ){  
                        printf("found on side1 line 109\n");
                    }
                    else if( (side3[k][0]==res2 && side3[k][1]==res1)  ){  
                ...
                if (flag==1){
                    ...
                    side5[47-i][0]=nodes_ptr[res2-1][res1-1];
                    side5[47-i][1]=res2;
                }
        ...
        /*Third side */
            ...
            if (i==0){
                ...
                for (k=0; k<i; k++){
                    if ( (side3[k][0]==res2 && side3[k][1]==res1)  ){
                        flag=1;
                    }
                    else if( (side2[k][0]==res2 && side2[k][1]==res1)  ){  
                        printf("found on side2 line 162\n");
                    }
                    else if( (side1[k][0]==res2 && side1[k][1]==res1)  ){  
                        printf("found on side1 line 165\n");
                    }
                }  
                if (flag==1){
                    side6[i][0]=res1;
                    side6[i][1]=nodes_ptr[res2-1][res1-1];
                    side6[47-i][0]=nodes_ptr[res2-1][res1-1];
                    side6[47-i][1]=res2;
                }
                else{
                    nodes_up[j][0]=(nodes[res1-1][0] + nodes[res2-1][0])/2;
    ...
    fp=fopen("con.dat", "w+");
    for (j=0; j<96; j++){
     fprintf(fp,"%d %d %d\n",connect_up[j][0],connect_up[j][1], connect_up[j][2]);
    }
    fclose(fp);
   
   fp=fopen("nodes.dat", "w+");
    for(j=0; j<65; j++){
